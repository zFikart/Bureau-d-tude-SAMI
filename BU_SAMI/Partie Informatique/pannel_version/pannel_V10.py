...
import numpy as np
from math import atan2, tanh, cos, sin, sqrt, pi
...
class Application(ttk.Frame):
    ...
    def __init__(self, *, numPC = 3, numRobot = 3, master = None, list_coord:list):
        ...
        self.image_path = "cocotte.jpg"
        self.image = plt.imread(self.image_path)
        self.imagebox = OffsetImage(self.image, zoom = 0.05)
    ...
    def initialize_updates_intervals(self):
        self.update_interval_aff_coord = 0.15         # en s 0.15 c'est bien
...
##
    ...
    def aff_coord(self):
        if self.mocap_node:
            ...
            for i in range(len(self.list_coord)):
                ...
                d = self.controlleur_bis(self.list_coord[i][0], self.list_coord[i][1],self.angle[0], self.x ,self.y)[2]
                print(f" POINT numéro {i} ".center(50, "="))
                while d > 0.05 :
                    ...
                    # pwm_g, pwm_d, d = self.controlleur(self.list_coord[i][0],self.list_coord[i][1], self.angle[0], self.x, self.y)
                    pwm_g, pwm_d, d = self.controlleur_bis(self.list_coord[i][0],self.list_coord[i][1], self.angle[0], self.x, self.y)
                    vg, vd = self.vitesse_normalisee(pwm_g*self.Ks), self.vitesse_normalisee(pwm_d*self.Ks)
                    # print(f"{d = }")
                    msg = f"DEPLACER {vg} {vd} {self.update_interval_aff_coord}"
                    self.client.send(msg)
                    print(f"Temps : {nb_sec:10.7f} ; pos : ({self.position_tuple[0]:.5f}, {self.position_tuple[1]:.5f}) ; rot : {self.angle[0]:.5f} ; d : {d:.3f}")
                ...
                # trace(f"\t{self.position_tuple}")
                    ...
                    # time.sleep(0.02)
                    time.sleep(self.update_interval_aff_coord + 0.05)


    def update_coord(self):
        self.lab_coord_pos.config(text = f"({self.position_tuple[0]:.5f}, {self.position_tuple[0]:.5f})")
        self.lab_coord_rot.config(text = f"{self.angle[0]:.5f}")
    ...
    def update_graph(self, i):
        ...
        #
        # end_x = self.x + self.arrow_length*cos(self.angle[0])
        # end_y = self.y + self.arrow_length*sin(self.angle[0])
        #
        # # self.ax = self.fig.add_subplot(111)
        # # self.ax.set_xlim(0, 3)
        # # self.ax.set_ylim(0, 3)
        # self.ax.quiver(self.x, self.y, end_x - self.x, end_y - self.y, angles = 'xy', scale_units = 'xy', scale = 1, color = 'r', width = 0.005)
        # self.ad = AnnotationBbox(self.imagebox, (self.x,self.y), frameon = False)
        #
        # self.ax.add_artist(self.ad)
        self.ax.set_title(f"Position: ({self.x:.5f}, {self.y:.5f})\nTheta : {self.angle[0]:.5f}")
        # self.ax.set_title(f"Position: ({x:.5f}, {y:.5f})")
        self.canvas.draw()
        # self.after(self.update_interval_animation, self.update_graph)
    ...
    def controlleur(self, xd,yd,theta,x,y) -> tuple:
        x_thilde = xd - x
        y_thilde = yd - y
        theta_thilde = atan2(y_thilde, x_thilde) - atan2(sin(theta), cos(theta))

        d = max(sqrt(x_thilde**2 + y_thilde**2), 0.05) # remplace condition
        v = (self.k1*d)/cos(theta_thilde)
        # if d < 0.05 :
        #     w = (v/0.05)*sin(theta_thilde)+ self.k2*theta_thilde
        # else  :
        #     w = (v/d)*sin(theta_thilde) + self.k2*theta_thilde
        w = (v/d)*sin(theta_thilde) + self.k2*theta_thilde
    ...
    def controlleur_bis(self, xd, yd, theta, x, y) -> tuple:
        k1 = 0.01
        k2 = 1.85*3
        k3 = 2
        vmax = 0.49
        ...
        y_thilde = yd - y
        theta_thilde = atan2(y_thilde, x_thilde) - atan2(sin(theta), cos(theta))
        d = max(sqrt(x_thilde**2 + y_thilde**2), 0.05)
        v = cos(theta_thilde)*vmax*d/(k1+d)
        w = sin(theta_thilde)*cos(theta_thilde)*vmax/(k1+d)+k2*tanh(theta_thilde*k3)
        PW_g = (v-delta*w/2)/self.Ks
        PW_d = (v+w*delta/2)/self.Ks
        return (PW_g,PW_d,d)


    def vitesse_normalisee(self, v, vmin = -1050, vmax = 1050) -> float:
        """Conversion vitesse m/s en rad/s"""
        r = 0.027 # m
        return min(max(360*v/(2*pi*r), vmin), vmax)
    ...
    def create_widgets(self):
        ...
        left_frame = tk.Frame(self, bg = bg_choisi)
        ...
        # end_x = x + arrow_length*cos(angle_rad)
        # end_y = y + arrow_length*sin(angle_rad)

        self.ax = self.fig.add_subplot(111)
        self.ax.set_xlim(0, 3)
        self.ax.set_ylim(0, 3)
        # self.ax.quiver(x, y, end_x - x, end_y - y, angles = 'xy', scale_units = 'xy', scale = 1, color = 'r', width = 0.005)
        # self.ad = AnnotationBbox(imagebox, (x,y), frameon = False)
        #
        # self.ax.add_artist(self.ad)
        ...
        cocotte_frame = ttk.Frame(left_frame,style = "Custom.TFrame")
        # Charger l'image de fond
        image_path  = r"O:\Documents\S6\UE_BE_Bureau_detude\PNG\cocotte.jpg"
        img = Image.open(image_path)
        photo = ImageTk.PhotoImage(img)
        cocotte_frame.photo = photo  # Conservation d'une référence à l'image
        # Créer un canvas tkinter et ajouter l'image de fond
        canvas = tk.Canvas(cocotte_frame, width=img.width, height=img.height)
        canvas.pack()
        ...
        cocotte_frame.pack(side = "top",fill = tk.X, padx = 5, pady = 5)
...
if __name__ == "__main__":
    ...
    Pos_bis = create_points(6, min = 0.1, max = 2.9)
    final_hull = insertion_optimisee(Pos_bis)
    print("Points de l'enveloppe convexe finale :", final_hull)

    # Pos = couple_random(5)
    root = tk.Tk()
    app = Application(master = root,  numPC = 3, numRobot = 3, list_coord = final_hull)
    app.mainloop()
