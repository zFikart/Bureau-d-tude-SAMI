...
class Handler_TCPServer(socketserver.BaseRequestHandler):
    def executer_commande(self, msg):
        ...
        if valeurs[0] == "MOVE":
            ...
            m_gauche.run_timed(speed_sp = int(valeurs[1]), time_sp = int(valeurs[1]))
            m_droite.run_timed(speed_sp = int(valeurs[2]), time_sp = 200)

        elif valeurs[0] == "BACKWARD":
            # mdiff.on_for_distance(SpeedPercent(int(valeurs[1]), 200))
            m_gauche.run_timed(speed_sp = -int(valeurs[1]), time_sp = 200)
            m_droite.run_timed(speed_sp = -int(valeurs[2]), time_sp = 200)
